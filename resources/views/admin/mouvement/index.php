...
@section('title')
    ...
    Comptabilite | Tous les mouvements
@endsection
                        ...
                        <table class="table-responsive-lg table display dataTablesCard student-tab dataTable no-footer" id="example">
                            ...
                            @foreach( $data as $mouvement  )
                                ...
                                <tr>
                                    ...
                                    <td><h6 class="mb-0">{{ $mouvement ['date_operation'] }} </h6></td>
                                    <td><h6 class="mb-0">
                                    ...
                                    <td><h6 class="mb-0">{{ $mouvement ['caisse'] }} </h6></td>

                                    @if( $role == \App\Types\Role::DIRECTEUR || $role == \App\Types\Role::ADMIN )
                                    <td><h6 class="mb-0">{{ $mouvement ['responsable'] }} </h6></td>
                                    ...
                                    <td>
                                        ...
                                        @if($mouvement ['type_mouvement'] === \App\Types\TypeMouvement::ENCAISSEMENT)

                                            <span class="badge badge-primary light badge-sm">Encaissement  <span
                                                    class="ms-1 fa fa-redo"></span></span>

                                        @endif

                                        @if($paiement ['statut_paiement'] === \App\Types\TypeMouvement::DECAISSEMENTS)

                                                <span class="badge badge-success light badge-sm">Décaissement  <span
                                        ...
                                        <div class="d-flex">
                                            <div class="dropdown custom-dropdown ">
                                                ...
                                                <div class="dropdown-menu dropdown-menu-end">
                                                    <a class="dropdown-item detailmouvement" href="" data-id="{{$mouvement['id']}}">Voir détail </a>


                                                        <a class="dropdown-item demanderAnnulation" href="#"  data-id="{{$mouvement['id']}}">Demander annulation </a>
...
@section('js')
    ...
    <script>
        jQuery(document).ready(function() {
            ...
            $(document).on('click', '.detailmouvement', function(event) {


                event.preventDefault();
                let id = $(this).data('id');
                let url = "/mouvements/detail/" + id;
                ...
                $.get(url, function(data) {
                    ...
                    $('#defaultModalLabel').text('Détail d un mouvement  ');

                    let detail_modal = $('#detailmouvement');

                    $(detail_modal).find('form').find('input[name="reference"]').val(data.paiement.reference);
                    $(detail_modal).find('form').find('input[name="beneficiaire"]').val(data.mouvement.beneficiaire);
                    $(detail_modal).find('form').find('input[name="montant"]').val(data.mouvement.montant);
                    $(detail_modal).find('form').find('input[name="caisse"]').val(data.mouvement.caisse.libelle);
